...
pub async fn login(
    ...
    let current_time: String = get_current_datetime().await;
    ...
    let new_login: String =
        login.as_ref().to_owned() + " 10=" + &(calculate_checksum(&login).await) + "|";
    new_login
}

async fn get_current_datetime() -> String {
    let current_time: DateTime<Utc> = Utc::now();
    format_time(current_time).await
}

async fn format_time(current_time: DateTime<Utc>) -> String {
...
async fn calculate_checksum(message: &str) -> String {
